...
- **Active Sports (Season-Aware)**: Sports nav is driven by three lists in `shared/platform-config.ts`. `ACTIVE_SPORTS` (SOCCER, MLB, GOLF) are in-season and fully available. `COMING_SOON_SPORTS` (NFL, NBA) are off-season but shown in nav with a "Fall 2026" badge; NFL links to a preview-mode `/nfl-mme` MME Optimizer page. `OFF_SEASON_SPORTS` (NHL) are hidden from nav and disabled in the frontend. Backend seeding/ingest still loads NFL/NHL data for continuity, but only `ACTIVE_SPORTS` surface as fully-available frontend selectors. Primary focus is Soccer and Baseball.
- **Soccer = FIFA World Cup**: Soccer is configured for the FIFA World Cup. The Odds API key is `soccer_fifa_world_cup`, and ESPN news/scores use the `soccer/fifa.world` league slug. Player prop markets are `player_shots` and `player_shots_on_target`. DraftKings serves whatever soccer slate is live.
...
- **Boost Engine**: Data-driven scoring using player history, trends, salary movement, floor/ceiling projections, momentum, team environment, and sport-specific stacking. Boost multiplier: `boostScore * 0.008` capped at ±8%. Boosts are computed for all DK slates (main + alternates) and main slates on other platforms. Includes actual-performance anchor: when a player's recency-weighted actual average (0.85 decay, last 10 games) significantly trails their projection, positive boosts are dampened (<70% of proj → 30% of boost, <85% → 55%, <95% → 75%). Requires ≥3 actual data points to activate.
...
- **Sim Pool Parity**: Both Pro Optimizer Sim and Vault ReSim use the full player pool (no artificial trimming) with identical projection pipelines (boosts, ceiling, leverage, historical, DvP, Vegas). Sim Regenerate LP solves use 70/30 blended sim/base projections to anchor candidates near optimizer quality, with an auto-calculated projection floor (85% of baseline optimal) filtering weak candidates.
...
- **Actual Points Backfill**: Standalone ESPN-based actual fantasy points backfill for player history (7-day rolling window, including today). Runs on startup and daily at 3:15 AM ET, independent of winning lineup analysis. Supports NBA, NHL, MLB, NFL. Includes retry logic for ESPN API calls.
- **Actual Points Gap-Fill**: Cron job at 9 AM, 3 PM, and 9 PM ET scans the last 14 days for sport/date combinations with less than 95% coverage and re-fetches ESPN data to fill gaps. Only NULL actual_points are treated as missing (resolved zero-point games are preserved).
...
- **Player Configuration**: Per-user, per-slate overrides for custom projections, boosts, locks, exclusions, and per-player min/max exposure limits. Exposure overrides persist and apply automatically across all optimizers (Pro, Sim, Bulk Regenerate, Sim Regenerate). Includes platform selector (DraftKings/FanDuel/Yahoo) defaulting to DraftKings. Uses `includeStarted=true` to show all active slates regardless of grace period. Slate selector dropdown filters displayed players by specific slate (shown when multiple slates exist for the platform), while all overrides save universally to the main slate via DraftKings player ID cross-reference.
