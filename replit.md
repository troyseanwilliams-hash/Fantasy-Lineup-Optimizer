...
8. **prizepicks_entries** - Saved PrizePicks entries per user (userId, sport, picks jsonb, multiplier, wager, potentialPayout, label, overallConfidence, status, createdAt). Pro-only vault with max 50 entries per user. API: `GET/POST /api/prizepicks/vault/entries`, `DELETE /api/prizepicks/vault/entries/:id`
