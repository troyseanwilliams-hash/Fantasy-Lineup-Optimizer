...
- **Active Sports**: NBA is currently disabled (not fetched, seeded, ingested, or shown in the UI). Active sports are SOCCER, MLB, GOLF, NHL (controlled by `ACTIVE_SPORTS` in `shared/platform-config.ts`). NFL data still loads in the backend but is not surfaced in the frontend selectors. Primary focus is Soccer and Baseball.
