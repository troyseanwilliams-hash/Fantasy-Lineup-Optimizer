...
import { Zap, Trophy, TrendingUp, ChevronRight, Loader2, Calendar, Clock } from "lucide-react";
...
export default function Home() {
  ...
  const nbaSlates = slates?.filter(s => s.sport === "NBA") || [];
  const nflSlates = slates?.filter(s => s.sport === "NFL") || [];
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      <div className="mb-12">
        ...
        <p className="text-lg text-slate-400">Select a slate to start building lineups.</p>
      </div>
      ...
      {nbaSlates.length > 0 && (
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-2xl font-black text-white">NBA Slates</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {nbaSlates.map(slate => (
              <Link key={slate.id} href={`/optimizer/${slate.id}`}>
                <Card className="group cursor-pointer bg-slate-800/30 border-slate-800 hover:border-emerald-500/40 p-6 transition-all duration-300 hover:shadow-xl hover:shadow-emerald-500/5" data-testid={`slate-card-${slate.id}`}>
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <span className="inline-block bg-emerald-500/10 text-emerald-400 text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider mb-2">
                        {slate.sport}
                      </span>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-800 flex items-center text-emerald-400 text-sm font-bold">
                    <Zap className="w-4 h-4 mr-2 fill-current" />
                    Open Optimizer
                  </div>
                </Card>
      ...
      {nflSlates.length > 0 && (
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-2xl font-black text-white">NFL Slates</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {nflSlates.map(slate => (
              <Link key={slate.id} href={`/optimizer/${slate.id}`}>
