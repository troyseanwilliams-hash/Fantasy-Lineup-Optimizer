...
export default function ShowdownBuilder() {
  ...
  const { data: slates, isLoading: slatesLoading } = useQuery<Slate[]>({
    ...
    // Slates don't change after they're loaded for the day.
    // staleTime prevents refetching on window focus from blowing away state.
    // refetchInterval polls every 30 min in case a new slate is added,
    // but won't clear generated lineups (see useEffect below).
    staleTime: 1000 * 60 * 30,
    refetchInterval: 1000 * 60 * 30,
    refetchOnWindowFocus: false,
  });

  // Auto-select the main slate only when no slate is selected yet.
  // Runs when slates data first loads, not on every refetch.
  // Critically: never resets selectedSlateId once one is chosen —
  // that's what was blowing away generated lineups after lock.
  useEffect(() => {
    if (slates && slates.length > 0 && !selectedSlateId) {
      const main = slates.find(s => s.isMain) || slates[slates.length - 1];
      setSelectedSlateId(main.id);
    }
  }, [slates]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const { data: playerData, isLoading: playersLoading } = useQuery<{ slate: any; games: Record<string, Player[]>; players: Player[] }>({
    ...
    // Prevent refetch on window focus from disrupting custom projections or
    // blowing away player data mid-session after slate lock.
          ...
          <Card className="bg-slate-900 border-slate-700">
            <CardContent className="p-12 text-center" data-testid="showdown-select-slate">
              ...
              <h3 className="text-lg font-bold text-white mb-2">
                {(slates || []).length === 0 ? `No ${sport} Slate Today` : "Select a Slate"}
              </h3>
              <p className="text-slate-400">
                {(slates || []).length === 0
                  ? `No ${sport} showdown slate is available right now. Check back later.`
                  : `Choose a ${sport} slate above to start building showdown lineups.`}
              </p>
            </CardContent>
          </Card>
        ) : selectedSlateId && playersLoading && generatedLineups.length === 0 ? (
          // Only block the UI with a spinner if we have no lineups to show yet.
          // Background refetches (e.g. projection updates) won't interrupt the session.
          <div className="flex items-center justify-center py-20">
            <Loader2 className="w-8 h-8 text-amber-500 animate-spin" />
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            {/* Slate locked banner — shown after startTime passes, keeps content visible */}
            {(() => {
              const activeSlate = (slates || []).find(s => s.id === selectedSlateId);
              const isLocked = activeSlate && new Date(activeSlate.startTime) <= new Date();
              if (!isLocked) return null;
              return (
                <div className="lg:col-span-5 bg-amber-500/10 border border-amber-500/25 rounded-xl px-4 py-2.5 flex items-center gap-2" data-testid="slate-locked-banner">
                  <span className="text-amber-400 text-sm">🔒</span>
                  <p className="text-amber-300 text-xs font-bold">
                    This slate has locked — games are in progress. Your lineups are preserved until 3 AM ET reset.
                  </p>
                </div>
              );
            })()}
