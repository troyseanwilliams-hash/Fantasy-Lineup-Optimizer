...
optimizerRouter.post("/api/optimize/sim", async (req, res) => {
  ...
  try {
    ...
    // Seed every pool player with their base projection
    for (const p of pool) {
    ...
    // ── Fetch Vegas + DvP contexts in parallel (max 3s each) ─────────────────
    ...
    // ── Apply DvP multipliers to base projections ─────────────────────────────
    // DvP shifts the median each player sims around. Vegas widens/narrows the
    // variance of that sampling. These are independent, compounding effects.
    ...
    res.json({
      ...
      // Diagnostic: what data enriched this run
      context: {
        vegasApplied,
        ...
        dvpApplied,
        dvpTeamsFound:    dvpContext ? dvpContext.size / 8 : 0,  // rough team count
      },
    });
  } catch (err) {
    if (err instanceof z.ZodError) return res.status(400).json({ message: err.errors[0].message });
