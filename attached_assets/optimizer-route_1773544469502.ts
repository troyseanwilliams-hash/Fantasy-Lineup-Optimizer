...
const baseOptimizeFields = {
  ...
  // Floor: only lineups whose total projected score >= this value are returned.
  // 0 results is a valid 200 response (floor was simply too high).
...
interface SolveOptions {
  ...
  // Exposure tracking for multi-lineup
...
function getEffectiveProjection(
  ...
  let proj = projectionMode === "ceiling"
...
function solveLineup(opts: SolveOptions): { lineup: Player[]; totalSalary: number; totalProjectedPoints: number; key: string } | null {
  const { pool, config, sport, lockedPlayerIds, projOverrides, projectedPointsFloor,
          leverageMode, projectionMode, ownershipOverrides, excludeKeys,
  ...
  eligiblePool.forEach(p => {
    const effProj = getEffectiveProjection(p, projOverrides, projectionMode, leverageMode, ownershipOverrides);
    const vName = `p${p.id}`;
  ...
  const totalProj   = lineup.reduce((s, p) => s + getEffectiveProjection(p, projOverrides, projectionMode, leverageMode, ownershipOverrides), 0);
