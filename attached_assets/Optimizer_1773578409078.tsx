...
export default function Optimizer() {
  ...
  // ── Feature 2: Projection accuracy from winning lineup history ───────────
  const { data: accuracyData } = useQuery<{ players: Array<{ playerName: string; hitRate: number; avgDelta: number; slatesAnalyzed: number }> }>({
    ...
    enabled: !!sport && !!players && players.length > 0,
  ...
  function getPlayerAccuracy(name: string) {
    const key = name.toLowerCase().replace(/[^a-z\s]/g, "").replace(/\s+/g, " ").trim();
    return hitRateMap.get(key) || null;
  }
    ...
    <div className="flex flex-col xl:flex-row h-[calc(100vh-80px)] overflow-hidden">
          ...
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead className="sticky top-0 z-10 bg-slate-900 border-b border-slate-800">
              <tr className="text-slate-400">
                ...
                <th className="px-3 py-3 text-[11px] font-black uppercase tracking-widest w-14 text-center hidden xl:table-cell" title="Historical hit rate from winning lineup data">Hit%</th>
                ...
                return (
                  <tr key={player.id} className={`border-b border-slate-800/50 transition-all group ${isIneligible ? "opacity-30" : isEligibleReplacement ? `cursor-pointer ${platform === "fanduel" ? "hover:bg-blue-500/5" : "hover:bg-emerald-500/5"}` : isLocked ? `${platform === "fanduel" ? "bg-blue-500/5" : "bg-emerald-500/5"}` : isExcluded ? "bg-red-500/5 opacity-50" : isInLineup ? "bg-slate-800/30" : "hover:bg-slate-800/30"}`} data-testid={`player-row-${player.id}`} onClick={isEligibleReplacement ? () => handleSelectReplacement(player) : undefined}>
                    ...
                    <td className="px-3 py-2 text-center hidden xl:table-cell">
                      {(() => {
                        const acc = getPlayerAccuracy(player.name);
                        if (!acc) return <span className="text-slate-700 text-[10px]">—</span>;
                        const color = acc.hitRate >= 60 ? "text-emerald-400 bg-emerald-500/10 border-emerald-500/20"
                                    : acc.hitRate >= 45 ? "text-slate-300 bg-slate-700/40 border-slate-600/30"
                                    : "text-red-400 bg-red-500/10 border-red-500/20";
                        return (
                          <span
                            className={`text-[10px] font-black px-1.5 py-0.5 rounded border ${color}`}
                            title={`Beats projection ${acc.hitRate}% of the time over ${acc.slatesAnalyzed} slates. Avg delta: ${acc.avgDelta > 0 ? "+" : ""}${acc.avgDelta} pts`}
                            data-testid={`hit-rate-${player.id}`}
                          >
                            {acc.hitRate}%
                          </span>
                        );
      ...
      <div className={`w-full xl:w-[420px] flex flex-col bg-slate-900/30 border-l border-slate-800 overflow-hidden ${mobileView !== "lineup" ? "hidden xl:flex" : ""}`}>
        ...
        <div className="px-3 sm:px-4 py-2 sm:py-3 bg-slate-900/60 border-b border-slate-800">
          ...
          {/* ── Cash / GPP toggle ─────────────────────────────────────────── */}
          <div className="flex items-center gap-1.5 bg-slate-800/60 rounded-lg p-1 border border-slate-700/50" data-testid="contest-type-toggle">
            <button
              onClick={() => setContestType("cash")}
              className={`flex-1 py-1.5 rounded-md text-xs font-black transition-all ${
                contestType === "cash"
                  ? `${platform === "fanduel" ? "bg-blue-500 shadow-blue-500/20" : "bg-emerald-500 shadow-emerald-500/20"} text-white shadow-md`
                  : "text-slate-400 hover:text-slate-200"
              }`}
              data-testid="btn-contest-cash"
              title="Cash: targets high-floor, consistent plays. Safe for 50/50s and head-to-heads."
            >
              💵 Cash
            </button>
            <button
              ...
              className={`flex-1 py-1.5 rounded-md text-xs font-black transition-all ${
                contestType === "gpp"
                  ? "bg-amber-500 shadow-amber-500/20 text-white shadow-md"
                  : "text-slate-400 hover:text-slate-200"
              }`}
              data-testid="btn-contest-gpp"
              title="GPP: targets high-ceiling differentiated plays. Best for large-field tournaments."
            >
              🏆 GPP
            </button>
          </div>
          <p className="text-[10px] text-slate-500 -mt-1 text-center">
            {contestType === "cash"
              ? "High-floor plays — targets safe, consistent production"
              : "High-ceiling plays — targets upside for tournament differentiation"}
          </p>
