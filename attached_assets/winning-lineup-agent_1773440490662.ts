...
// buildPositionVariables maps a player's position string to LP constraint
// contribution variables. Each key corresponds to a constraint in the LP model:
//   - Named slot keys (PG, SG, C, W, etc.): exact slot eligibility
//   - Aggregate keys (G, F, SKATER, FLEX, OUTFIELD): cross-slot totals
//
// IMPORTANT: Every key emitted here must have a corresponding constraint in the
// LP model. Aggregate constraints come from config.aggregateConstraints;
// named slot constraints come from config.positionConstraints.
function buildPositionVariables(position: string, sport: string): Record<string, number> {
  ...
  switch (sport) {
    ...
    case "MLB":
      ...
      if (positions.includes("OF") || positions.includes("LF") || positions.includes("CF") || positions.includes("RF")) { vars.OF = 1; }
      break;
    ...
    case "SOCCER":
      ...
      if (positions.includes("M") || positions.includes("MF")) { vars.M = 1; vars.OUTFIELD = 1; }
...
/**
 * Normalizes a player name for fuzzy matching between our DB and
 * external actual-points APIs. Strips diacritics, punctuation,
 * and extra whitespace so "Nikola Jokić" matches "Nikola Jokic".
 */
function normalizeName(name: string): string {
  return name
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "") // strip diacritics (Jokić → Jokic)
    .toLowerCase()
    .replace(/['']/g, "")            // curly apostrophes
    .replace(/[^a-z\s]/g, "")        // all remaining non-alpha
    .replace(/\s+/g, " ")
    .trim();
}
...
function solveOptimalLineup(pool: PlayerWithActual[], sport: string): {
  ...
  // Named slot constraints (PG≥1, C≥1, W≥3, etc.)
  // These correspond directly to position keys in buildPositionVariables.
  for (const [key, constraint] of Object.entries(config.positionConstraints)) {
    model.constraints[key] = constraint;
  }

  // Aggregate constraints (G≥3 for NBA, SKATER≥8 for NHL, etc.)
  // These use the same variable keys emitted by buildPositionVariables
  // but represent cross-slot totals rather than individual slot minimums.
  // Previously these were mixed into positionConstraints, which caused
  // NBA to be infeasible: G≥3 + F≥3 on top of PG/SG/SF/PF/C named slots
  // required 10 players on an 8-man roster.
  ...
  if (!result.feasible) {
    return {
      error: "Could not find feasible optimal lineup — check pool size and position coverage",
      lineup: [],
      totalSalary: 0,
      totalActualPoints: 0,
    };
  ...
  if (selectedPlayers.length !== config.rosterSize) {
    return {
      error: `Roster size mismatch: got ${selectedPlayers.length}, expected ${config.rosterSize}`,
      lineup: [],
      totalSalary: 0,
      totalActualPoints: 0,
    };
...
function computeInsights(lineup: PlayerWithActual[], pool: PlayerWithActual[], sport: string) {
  ...
  const positionBreakdown: Record<string, {
    count: number; avgSalary: number; avgActual: number; avgProjected: number;
  }> = {};
  ...
  const projectionAccuracy = lineup.map(p => ({
    ...
    ratio:     Number(p.projectedPoints) > 0
      ? Math.round((p.actualPoints / Number(p.projectedPoints)) * 100) / 100
      : 0,
  }));

  const avgProjectionRatio = projectionAccuracy.length > 0
    ? Math.round(
        (projectionAccuracy.reduce((s, p) => s + p.ratio, 0) / projectionAccuracy.length) * 100
      ) / 100
    : 0;
  ...
  const poolAvgActual        = pool.length > 0
    ? pool.reduce((s, p) => s + p.actualPoints, 0) / pool.length
    : 0;
  const lineupAvgActual      = totalActualPoints / lineup.length;
  const outperformanceMultiple = poolAvgActual > 0
    ? Math.round((lineupAvgActual / poolAvgActual) * 100) / 100
    : 0;
...
export async function analyzeCompletedSlate(
  sport: string,
  slateDate: string
  ...
  try {
    ...
    // Deduplicate by name + team only (not position) so that multi-position
    // players stored with different position strings aren't treated as
    // separate players and double-selected by the LP solver.
    const deduped = new Map<string, typeof allSlateRecords[0]>();
    for (const h of allSlateRecords) {
      const key = `${h.playerName}_${h.team}`;
    ...
    console.log(
      `[WinningAgent] ${sport} ${slateDate}: ${allSlateRecords.length} total records, ` +
      `${slatePlayers.length} unique players after dedup`
    );
    ...
    console.log(
      `[WinningAgent] ${sport} ${slateDate}: Matched ${matchCount}/${slatePlayers.length} players with actual points`
    );

    if (batchUpdates.length > 0) {
      await storage.batchUpdatePlayerHistoryActualPoints(sport, slateDate, batchUpdates);
      console.log(
        `[WinningAgent] ${sport} ${slateDate}: Updated ${batchUpdates.length} player history records with actual points`
      );
    }

    // Require at least a full roster worth of matched players before solving.
    // Using rosterSize rather than a hardcoded 10 so NBA (8) doesn't fail
    // when only 8-9 players matched (which is a valid complete lineup).
    const config = getPlatformConfig(sport, "draftkings");
    if (matchCount < config.rosterSize) {
      return {
        success: false,
        message: `Only matched ${matchCount} players with actual data — need at least ${config.rosterSize} for ${sport}`,
      };
    }

    // Exclude players with zero actual points — these are players who didn't play
    // (scratched, DNP, etc.) rather than players who played and scored 0.
    // Note: a player who played and scored exactly 0 DK points will also be excluded.
    // This is an acceptable tradeoff since including DNPs would distort the optimal lineup.
    ...
    const record: InsertWinningLineup = {
      sport,
      // slateId from first record is best-effort — multiple slates for same sport/date
      // would require a smarter lookup here.
...
export async function runNightlyAnalysis(): Promise<string[]> {
  ...
  // Use ACTIVE_SPORTS from platform-config rather than a hardcoded list.
  // Previously hardcoded to ["NBA", "NHL", "MLB", "NFL"] which skipped
  // GOLF and SOCCER while running MLB and NFL in their off-seasons.
  const sports = ACTIVE_SPORTS;

  const todayET      = getEasternToday(); // e.g. "2026-03-13"
  const d            = new Date(todayET + "T12:00:00Z");
  // Use setUTCDate to avoid server local timezone influencing the subtraction.
