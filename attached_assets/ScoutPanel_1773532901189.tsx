/**
 * EliteLineup.com — ScoutPanel Component
 *
 * Drop this panel into Optimizer.tsx and ProOptimizer.tsx to surface
 * AI Scout signals alongside the player pool.
 *
 * Usage in Optimizer.tsx (left column, above player table):
 *   import { ScoutPanel } from "@/components/ScoutPanel";
 *   <ScoutPanel sport={sport} players={players} onBoostApply={handleBoostApply} />
 *
 * Usage in ProOptimizer.tsx (right sidebar, above AI Boosts section):
 *   <ScoutPanel sport={sport} players={players} compact />
 */
...
interface ScoutPanelProps {
  ...
  players?: Player[];
  compact?: boolean;
  /** Called when user clicks "Apply Boosts" — passes merged projections map */
...
// BOOST_WEIGHTS mirrors the Python values — used to compute client-side projection delta
...
export function ScoutPanel({ sport, players, compact = false, onBoostApply }: ScoutPanelProps) {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setCountdown(prev => {
        if (prev <= 1) {
          // Auto-refetch when timer hits zero
  ...
  const refreshMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Scout Scanning", description: "AI is scrubbing the web for latest news..." });
      setTimeout(() => {
        queryClient.invalidateQueries({ queryKey: ["/api/scout/signals", sport] });
        queryClient.invalidateQueries({ queryKey: ["/api/scout/status"] });
      }, 4000);
  ...
  return (
    <div className="border border-slate-700/50 rounded-xl overflow-hidden bg-slate-900/60 mb-3">
      {/* Header */}
      <button
        ...
        <div className="border-t border-slate-700/50">
          ...
          <div className="max-h-64 overflow-y-auto divide-y divide-slate-800/60">
            ...
            {filteredSignals.slice(0, compact ? 6 : 20).map((sig, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-2.5 px-3 py-2.5 hover:bg-slate-800/30 transition-colors">
...
// ─────────────────────────────────────────────────────────────────────────────
// ScoutStatusBar — compact single-line bar for top of optimizer pages
// ─────────────────────────────────────────────────────────────────────────────
...
export function ScoutStatusBar({ sport }: { sport: string }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-1.5 bg-emerald-950/40 border-b border-emerald-900/30 text-[11px]">
