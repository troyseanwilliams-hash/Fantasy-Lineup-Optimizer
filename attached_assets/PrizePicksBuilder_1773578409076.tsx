...
export default function PrizePicksBuilder() {
  ...
  const [sortMode, setSortMode] = useState<"time" | "line-high" | "line-low">("time");
            ...
            <div className="flex gap-2 flex-col sm:flex-row">
              ...
              <div className="flex gap-1.5 flex-wrap items-center">
                ...
                <button
                  ...
                  className={`text-xs font-black px-2.5 py-1.5 rounded-lg border transition-all ${
                    movedOnly
                      ? "bg-amber-500/20 text-amber-400 border-amber-500/30"
                      : "bg-slate-800/50 text-slate-400 border-slate-700/50 hover:text-white"
                  }`}
                  data-testid="pp-builder-filter-moved"
                  title="Show only lines that have moved"
                >
                  {movedOnly ? "▲▼ Moved" : "▲▼ All Lines"}
                </button>
                <Button
                  size="sm"
                      ...
                      <div className="hidden sm:flex items-center gap-3">
                        ...
                        <div className="text-center shrink-0 mr-2">
                          ...
                          <div className="flex items-center justify-center gap-1">
                            ...
                            {(() => {
                              const mv = lineMovements[proj.id];
                              if (!mv) return null;
                              return (
                                <span className={`text-[9px] font-black px-1 py-0.5 rounded leading-tight ${
                                  mv.direction === "down"
                                    ? "text-emerald-400 bg-emerald-500/10"
                                    : "text-red-400 bg-red-500/10"
                                }`} title={`Line moved ${mv.direction === "down" ? "down" : "up"} ${Math.abs(mv.delta)} from ${mv.previousLine} (${mv.minutesAgo}m ago)`}>
                                  {mv.direction === "down" ? "▼" : "▲"}{Math.abs(mv.delta)}
                                </span>
                              );
                            })()}
                          </div>
                            ...
                            {(() => {
                              ...
                              return (
                                <span className={`text-[9px] font-black px-1 py-0.5 rounded ${
                                  mv.direction === "down" ? "text-emerald-400 bg-emerald-500/10" : "text-red-400 bg-red-500/10"
                                }`}>
                                  {mv.direction === "down" ? "▼" : "▲"}{Math.abs(mv.delta)}
                                </span>
                              );
